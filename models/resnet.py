...
class ResNet(nn.Module):
    ...
    def __init__(self, conv_layer, linear_layer, block, layers, width_per_group=64, num_classes=1000, zero_init_residual=False,
...
def WideResNet50_2(conv_layer, linear_layer, init_type, **kwargs):
    assert init_type == "kaiming_normal", "only supporting default init for Resnets"
    return ResNet(conv_layer, linear_layer, Bottleneck, [3, 4, 6, 3], width_per_group=64*2, **kwargs)

def WideResNet50_4(conv_layer, linear_layer, init_type, **kwargs):
    assert init_type == "kaiming_normal", "only supporting default init for Resnets"
    return ResNet(conv_layer, linear_layer, Bottleneck, [3, 4, 6, 3], width_per_group=64*4, **kwargs)

def WideResNet101_2(conv_layer, linear_layer, init_type, **kwargs):
    assert init_type == "kaiming_normal", "only supporting default init for Resnets"
    return ResNet(conv_layer, linear_layer, Bottleneck, [3, 4, 23, 3], width_per_group=64*2, **kwargs)