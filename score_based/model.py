...
from score_based.layers import SubnetConv, SubnetLinear

# Hydra from https://github.com/inspire-group/hydra
...
def set_pruning_type_model(model, pruning_type):
    for _, v in model.named_modules():
        if hasattr(v, "set_pruning_type"):
            v.set_pruning_type(pruning_type)
...
def prepare_model(model, args):
    ...
    set_prune_rate_model(model, args.k)
    set_pruning_type_model(model, args.pruning_type)
...
def current_model_pruned_fraction(model, result_dir, args, verbose=True):
    """
    ...
    path = os.path.join(result_dir, "checkpoint_dense_" + args.pruning_type + ".pth.tar")
